...
class Message
{
     ...
     * @param string        $type        消息类型（text, image, voice, file, video, mixed）
     ...
     * @param string[]      $videoUrls   视频 URL 列表
     ...
     * @param array<string, string> $videoAesKeys 视频 URL → AES Key 映射
    ...
    public function __construct(
        ...
        public readonly array $videoAesKeys = [],
     ...
     *
     * @param MediaDownloader|null $downloader 下载器实例，为 null 时自动创建
     * @return DownloadResult|null 无视频时返回 null
     */
    public function downloadVideo(?MediaDownloader $downloader = null): ?DownloadResult
    {
        ...
        $aesKey = $this->videoAesKeys[$url] ?? null;
     ...
     * @return DownloadResult[] 下载结果数组
     */
    public function downloadAllVideos(?MediaDownloader $downloader = null): array
    {
        $downloader = $downloader ?? new MediaDownloader();
        $results = [];
